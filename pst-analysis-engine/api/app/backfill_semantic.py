...
def backfill_emails(
    ...
    try:
        ...
        if not semantic_service.ensure_ready():
            logger.error("Failed to initialize semantic service - OpenSearch may be down")
    ...
    while offset < stats['total']:
        ...
        for email in batch:
            try:
                ...
                body_text = email.body_text_clean or email.body_text or email.body_preview
        ...
        logger.info(f"Progress: {offset}/{stats['total']} ({progress:.1f}%) - Processed: {stats['processed']}, Errors: {stats['errors']}")

...
from .models import EvidenceItem, EmailMessage, Case, Project
...
class EnhancedEvidenceProcessor:
    ...
    async def process_evidence_item(self, evidence_id: str, db: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            evidence.extracted_parties = [e['Text'] for e in entities if e['Type'] == 'PERSON']
            evidence.extracted_dates = [e['Text'] for e in entities if e['Type'] == 'DATE']
            evidence.extracted_amounts = [e['Text'] for e in entities if e['Type'] in ['QUANTITY', 'OTHER'] and any(c in e['Text'] for c in ['£', '$', '€'])]
            
            # Auto-classify document type
            evidence.evidence_type = self._classify_document_type(textract_data, comprehend_analysis)
            evidence.classification_confidence = self._calculate_classification_confidence(textract_data, comprehend_analysis)
            
            # Generate auto-tags
            evidence.auto_tags = self._generate_auto_tags(textract_data, comprehend_analysis, image_analysis)
    ...
    async def process_email_thread(self, thread_id: str, db: Session) -> Dict[str, Any]:
        ...
        try:
            emails = db.query(EmailMessage).filter(EmailMessage.thread_id == thread_id).all()
            ...
            for email in emails:
                if email.body_text:
                    # Analyze each email
                    analysis = await self.aws.analyze_document_entities(email.body_text[:5000])
            ...
            sentiments = [s['sentiment'] for s in thread_analysis['sentiment_progression']]
    ...
    async def process_audio_evidence(self, s3_bucket: str, s3_key: str, evidence_id: str) -> Dict[str, Any]:
        """Process audio/video evidence with transcription"""
        try:
            job_name = f"vericase-{evidence_id}-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
    ...
    async def semantic_search(self, query: str, case_id: Optional[str] = None) -> List[Dict[str, Any]]:
        """Semantic search across all evidence using Bedrock KB"""
        try:
            # Query Bedrock Knowledge Base
            kb_results = await self.aws.query_knowledge_base(query, self.knowledge_base_id)
    ...
    async def generate_case_insights(self, case_id: str, db: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            insights['key_entities'] = dict(sorted(entity_counts.items(), key=lambda x: x[1], reverse=True)[:20])
            ...
            insights['ai_recommendations'] = [r.get('content', {}).get('text', '') for r in ai_recommendations[:3]]
    ...
    def _classify_document_type(self, textract_data: Dict, comprehend_analysis: Dict) -> str:
        """Classify document type based on extracted data"""
        text = textract_data.get('text', '').lower()
        queries = textract_data.get('queries', {})
        entities = comprehend_analysis.get('entities', [])
        
        # Contract indicators
        if any(word in text for word in ['contract', 'agreement', 'terms', 'conditions']):
            return 'contract'
        ...
        # Meeting minutes
        if any(word in text for word in ['minutes', 'meeting', 'attendees', 'action items']):
            return 'meeting_minutes'
        
        # Default
        return 'other'
    
    def _calculate_classification_confidence(self, textract_data: Dict, comprehend_analysis: Dict) -> int:
    ...
    def _generate_auto_tags(self, textract_data: Dict, comprehend_analysis: Dict, image_analysis: Dict) -> List[str]:
        ...
        if image_analysis.get('construction_elements'):
            tags.extend([f"visual:{element}" for element in image_analysis['construction_elements'][:5]])
        ...
        try:
            ...
            document_data = {
                ...
                'metadata': {
                    ...
                    'document_date': evidence.document_date.isoformat() if evidence.document_date else None,
                    'filename': evidence.filename,

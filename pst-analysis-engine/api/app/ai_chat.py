...
Provide a clear, concise answer citing specific emails. If the evidence doesn't support an answer, say so."""
    ...
    async def _create_evidence_plan(self, query: str, emails: List[EmailMessage]) -> ResearchPlan:
        ...
        if any(word in query_lower for word in ['chronology', 'timeline', 'sequence', 'when']):
            focus = "chronology"
        elif any(word in query_lower for word in ['delay', 'critical', 'programme', 'schedule']):
            focus = "delay_analysis"
        elif any(word in query_lower for word in ['narrative', 'story', 'explain', 'what happened']):
            focus = "narrative"
        elif any(word in query_lower for word in ['find', 'search', 'locate', 'show me']):
...
Provide a critical analysis identifying gaps and suggesting what's needed."""
            ...
            return ModelResponse(
                ...
                confidence=0.90,
                processing_time=processing_time,
                key_findings=self._extract_key_findings(response.choices[0].message.content)
            )
        except Exception as e:
    ...
    def synthesize_evidence_analysis(self, model_responses: List[ModelResponse], query: str) -> str:
        ...
        if not valid_responses:
            return "❌ All AI models failed. Please check your API keys in the .env file."
        
        if len(valid_responses) == 1:
            return f"## {valid_responses[0].task}\n\n{valid_responses[0].response}"
        
        # Build comprehensive synthesis
        synthesis = f"# Comprehensive Evidence Analysis\n\n"
        ...
        if all_findings:
            synthesis += f"## 🎯 Synthesized Key Findings\n\n"
...
async def ai_evidence_query(
    ...
    try:
        ...
        if request.mode == ResearchMode.QUICK:
            ...
            plan, model_responses = await orchestrator.deep_research(request.query, emails)
            
            # Synthesize
            synthesized = orchestrator.synthesize_evidence_analysis(model_responses, request.query)

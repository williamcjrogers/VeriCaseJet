...
async def list_contentious_matters(
    ...
    total = query.count()
    matters = query.order_by(ContentiousMatter.created_at.desc()).offset((page - 1) * page_size).limit(page_size).all()
...
async def list_heads_of_claim(
    ...
    if contentious_matter_id:
        query = query.filter(HeadOfClaim.contentious_matter_id == uuid.UUID(contentious_matter_id))
    if status:
    ...
    total = query.count()
    claims = query.order_by(HeadOfClaim.created_at.desc()).offset((page - 1) * page_size).limit(page_size).all()
    ...
    for c in claims:
        ...
        result.append(HeadOfClaimResponse(
            id=str(c.id),
            name=c.name,
            description=c.description,
            project_id=str(c.project_id) if c.project_id else None,
            case_id=str(c.case_id) if c.case_id else None,
            contentious_matter_id=str(c.contentious_matter_id) if c.contentious_matter_id else None,
...
async def create_head_of_claim(
    ...
    claim = HeadOfClaim(
        ...
        project_id=uuid.UUID(request.project_id) if request.project_id else None,
        case_id=uuid.UUID(request.case_id) if request.case_id else None,
        contentious_matter_id=uuid.UUID(request.contentious_matter_id) if request.contentious_matter_id else None,
...
async def get_head_of_claim(
    ...
    return HeadOfClaimResponse(
        ...
        contentious_matter_id=str(claim.contentious_matter_id) if claim.contentious_matter_id else None,
        contentious_matter_name=matter_name,
        reference_number=claim.reference_number,
...
async def list_item_links(
    ...
    if contentious_matter_id:
        query = query.filter(ItemClaimLink.contentious_matter_id == uuid.UUID(contentious_matter_id))
    if head_of_claim_id:
        query = query.filter(ItemClaimLink.head_of_claim_id == uuid.UUID(head_of_claim_id))
    if item_type:
    ...
    total = query.count()
    links = query.order_by(ItemClaimLink.created_at.desc()).offset((page - 1) * page_size).limit(page_size).all()
    ...
    for link in links:
        ...
        result.append(ItemLinkResponse(
            id=str(link.id),
            item_type=link.item_type,
            item_id=str(link.item_id),
            item_title=item_title,
            item_date=item_date,
            contentious_matter_id=str(link.contentious_matter_id) if link.contentious_matter_id else None,
            contentious_matter_name=matter_name,
            head_of_claim_id=str(link.head_of_claim_id) if link.head_of_claim_id else None,
...
async def create_item_link(
    ...
    if not request.contentious_matter_id and not request.head_of_claim_id:
        raise HTTPException(400, "Either contentious_matter_id or head_of_claim_id is required")
    ...
    existing = db.query(ItemClaimLink).filter(
        ...
        or_(
            and_(
                ItemClaimLink.contentious_matter_id == (uuid.UUID(request.contentious_matter_id) if request.contentious_matter_id else None),
                request.contentious_matter_id is not None
            ),
            and_(
                ItemClaimLink.head_of_claim_id == (uuid.UUID(request.head_of_claim_id) if request.head_of_claim_id else None),
    ...
    if existing:
        raise HTTPException(409, "This item is already linked to the specified matter/claim")
    
    link = ItemClaimLink(
        item_type=request.item_type,
        item_id=uuid.UUID(request.item_id),
        contentious_matter_id=uuid.UUID(request.contentious_matter_id) if request.contentious_matter_id else None,
        head_of_claim_id=uuid.UUID(request.head_of_claim_id) if request.head_of_claim_id else None,
...
async def get_link_comments(
    ...
    def build_comment_tree(comment):
        ...
        if comment.created_by:
            creator = db.query(User.email, User.display_name).filter(User.id == comment.created_by).first()
            if creator:
        ...
        return CommentResponse(
            ...
            item_claim_link_id=str(comment.item_claim_link_id) if comment.item_claim_link_id else None,
...
async def add_link_comment(
    ...
    # Verify link exists
    link = db.query(ItemClaimLink).filter(ItemClaimLink.id == uuid.UUID(link_id)).first()
    if not link:
        raise HTTPException(404, "Item link not found")
...
async def get_item_comments(
    ...
    link_ids = [l.id for l in link_ids]
    
    # Get direct comments and linked comments
    comments = db.query(ItemComment).filter(
        or_(
            and_(ItemComment.item_type == item_type, ItemComment.item_id == uuid.UUID(item_id)),
...
async def create_comment(
    ...
    comment = ItemComment(
        item_claim_link_id=uuid.UUID(request.item_claim_link_id) if request.item_claim_link_id else None,
        item_type=request.item_type,
...
async def get_claims_stats(
    ...
    link_query = db.query(func.count(ItemClaimLink.id)).filter(ItemClaimLink.status == 'active')
    ...
    if project_id:
        amount_query = amount_query.filter(HeadOfClaim.project_id == uuid.UUID(project_id))
    if case_id:

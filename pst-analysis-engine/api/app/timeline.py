...
def _adapt_programme_activities(
    ...
    for activity in programme.activities:
        ...
        try:
            ...
            end_dt = datetime.fromisoformat(finish_str.replace("Z", "+00:00")) if finish_str else None
        ...
        event_type = TimelineEventType.MILESTONE if is_milestone else TimelineEventType.ACTIVITY
        ...
        events.append(TimelineEvent(
            ...
            title=name,
            summary=f"{programme.programme_type.title()} - {name}" if programme.programme_type else name,
            start_date=start_dt,
...
def _adapt_delay_events(
    ...
    for delay in delays:
        ...
        event_date = delay.actual_finish or delay.planned_finish or delay.actual_start or delay.planned_start
        if not event_date:
            continue
        ...
        summary_parts = []
        if delay.delay_days:
            summary_parts.append(f"{abs(delay.delay_days)} day{'s' if abs(delay.delay_days) != 1 else ''} {'delay' if delay.delay_days > 0 else 'ahead'}")
        ...
        events.append(TimelineEvent(
            ...
            meta={
                ...
                "planned_start": delay.planned_start.isoformat() if delay.planned_start else None,
                "planned_finish": delay.planned_finish.isoformat() if delay.planned_finish else None,
                "actual_start": delay.actual_start.isoformat() if delay.actual_start else None,
...
def _aggregate_timeline_events(
    ...
    if TimelineEventType.ACTIVITY in include_types or TimelineEventType.MILESTONE in include_types:
        prog_query = db.query(Programme)
        if case_id:
            prog_query = prog_query.filter(Programme.case_id == uuid.UUID(case_id))
        if project_id:
            prog_query = prog_query.filter(Programme.project_id == uuid.UUID(project_id))
    ...
    if TimelineEventType.EMAIL in include_types:
        ...
        if filters.start_date:
            email_query = email_query.filter(EmailMessage.date_sent >= filters.start_date)
        if filters.end_date:
        ...
        emails = email_query.order_by(desc(EmailMessage.date_sent)).limit(min(filters.limit, 1000)).all()
    ...
    if TimelineEventType.DOCUMENT in include_types:
        ...
        if case_id:
            evidence_query = evidence_query.filter(EvidenceItem.case_id == uuid.UUID(case_id))
        if project_id:
            evidence_query = evidence_query.filter(EvidenceItem.project_id == uuid.UUID(project_id))
    ...
    if TimelineEventType.CHRONOLOGY in include_types:
        ...
        if case_id:
            chrono_query = chrono_query.filter(ChronologyItem.case_id == uuid.UUID(case_id))
        
        if filters.start_date:
            chrono_query = chrono_query.filter(ChronologyItem.event_date >= filters.start_date)
        if filters.end_date:
            chrono_query = chrono_query.filter(ChronologyItem.event_date <= filters.end_date)
    ...
    if filters.search:
        ...
        all_events = [
            ...
            if search_lower in e.title.lower() or (e.summary and search_lower in e.summary.lower())
        ]
...
def update_manual_event(
    ...
    if not event_id.startswith("chrono-"):
        raise HTTPException(
            status_code=400,
            detail="Only manual/chronology events can be updated"
        )
    ...
    try:
        item = db.query(ChronologyItem).filter(
            ChronologyItem.id == uuid.UUID(source_id)
        ).first()
    except ValueError:
...
def delete_manual_event(
    ...
    if not event_id.startswith("chrono-"):
        raise HTTPException(
            status_code=400,
            detail="Only manual/chronology events can be deleted"
        )

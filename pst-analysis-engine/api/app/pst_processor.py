...
class UltimatePSTProcessor:
    ...
    def process_pst(self, pst_s3_key: str, document_id: int, case_id: int | None = None, company_id: int | None = None, project_id: int | None = None) -> dict[str, Any]:
        ...
        if pst_file_record:
            ...
            if not document:
                raise ValueError(f"Neither PSTFile nor Document found for id={document_id}")
    ...
    def _process_folder(self, folder, pst_file_record, document, case_id, project_id, company_id, stats, folder_path=''):
        ...
        for i in range(num_messages):
            ...
            except (AttributeError, RuntimeError, OSError, ValueError, SystemError, MemoryError) as e:
                logger.warning(f"Skipping message {i} in {current_path}: {str(e)[:100]}")
    ...
    def _clean_body_text(self, text: str | None) -> str | None:
        ...
        text = re.sub(r'@[a-z-]+\s*\{[^}]*\}', '', text, flags=re.IGNORECASE)  # @media, @font-face, etc.
    ...
    def _process_message(self, message, pst_file_record, document, case_id, project_id, company_id, stats, folder_path):
        ...
        try:
            ...
            logger.warning(f"Could not get attachment count for message in {folder_path}: {e}")
        ...
        try:
            ...
            norm_to = ','.join(sorted([r.strip().lower() for r in to_recipients])) if to_recipients else ''
    ...
    def _get_attachment_filename(self, attachment: Any, index: int) -> str:
        ...
        filename = (
            self._safe_get_attr(attachment, 'name', None) or
            self._safe_get_attr(attachment, 'long_filename', None) or
            self._safe_get_attr(attachment, 'short_filename', None) or
            self._safe_get_attr(attachment, 'filename', None)
        )
        if filename:
            return filename
    ...
    def _get_attachment_content_type(self, attachment: Any, filename: str) -> str:
        ...
        content_type = (
            self._safe_get_attr(attachment, 'mime_type', None) or
            self._safe_get_attr(attachment, 'content_type', None)
        )
        if content_type:
            return content_type
    ...
    def _process_attachments(self, message: Any, email_message: EmailMessage, pst_file_record: Any, parent_document: Any, case_id: Any, project_id: Any, company_id: Any, stats: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for i in range(message.number_of_attachments):
            try:
                ...
                content_id = self._get_attachment_property(attachment, 0x3712)  # PR_ATTACH_CONTENT_ID
                ...
                if is_duplicate:
                    ...
                    att_doc = Document(
                        ...
                        owner_user_id=getattr(parent_document, 'owner_user_id', None) if parent_document else None,
                        meta={

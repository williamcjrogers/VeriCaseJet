...
def load_ai_keys_from_secrets_manager():
    ...
    try:
        ...
        if loaded_count > 0:
            logger.info(f"Successfully loaded {loaded_count} AI API keys from Secrets Manager")
            return True
        else:
        ...
        if error_code == 'ResourceNotFoundException':
            ...
            logger.warning(f"Access denied to secret {secret_name} - check IAM permissions")
        else:

...
from .models import EmailMessage, Programme, DelayEvent, Case, Document, User
...
@router.post("/api/programmes/auto-link-emails")
async def auto_link_emails_to_programmes(
    case_id: int,
    as_planned_id: int,
    as_built_id: int,
    db: Session = Depends(get_db),
    ...
    """
    Auto-link correspondence to programme activities using date overlap.

    - Finds activities active on the email sent date (inclusive).
    - Prefers critical path activities when multiple match.
    - Calculates delay_days from baseline vs as-built finish dates when IDs align.
    """

    as_planned = db.query(Programme).filter(Programme.id == as_planned_id).first()
    if not as_planned:
        raise HTTPException(status_code=404, detail="As-planned programme not found")

    as_built = db.query(Programme).filter(Programme.id == as_built_id).first()
    if not as_built:
        raise HTTPException(status_code=404, detail="As-built programme not found")

    # Ensure both programmes belong to the same case/project as the provided case_id
    if as_planned.case_id and as_planned.case_id != case_id:
        raise HTTPException(
            status_code=400,
            detail="As-planned programme does not belong to provided case_id",
        )
    if as_built.case_id and as_built.case_id != case_id:
        raise HTTPException(
            status_code=400,
            detail="As-built programme does not belong to provided case_id",
        )

    planned_activities = as_planned.activities or []
    built_activities = as_built.activities or []
    built_by_id = {
        a.get("id"): a for a in built_activities if isinstance(a, dict) and a.get("id")
    }
    ...
    def _activity_label(activity: dict[str, Any] | None) -> str | None:
        if not activity:
            return None
        act_id = activity.get("id", "")
        name = activity.get("name", "")
        label = f"{act_id} - {name}".strip(" -")
        return label or None

    # Align email selection with case/project linkage
    email_query = db.query(EmailMessage).filter(EmailMessage.case_id == case_id)
    # If programmes have a project_id set, further scope emails to that project to avoid cross-project linking
    if as_planned.project_id:
        email_query = email_query.filter(EmailMessage.project_id == as_planned.project_id)
    ...
    for email in emails:
        ...
        target_date = email.date_sent.replace(tzinfo=None)
        ...
        for activity in planned_activities:
            ...
            start = _parse_dt(activity.get("start_date"))
            finish = _parse_dt(activity.get("finish_date"))
            if not start or not finish:
                continue

            if start <= target_date <= finish:
                # Prefer critical path activities when multiple match
                if best_match is None:
                    best_match = activity
                elif activity.get("is_critical") and not best_match.get("is_critical"):
                    best_match = activity

        if best_match:
            email.as_planned_activity = _activity_label(best_match)

            built_match = built_by_id.get(best_match.get("id"))
            email.as_built_activity = _activity_label(built_match)

            # Calculate slippage if both finishes exist
            planned_finish = _parse_dt(best_match.get("finish_date"))
            built_finish = (
                _parse_dt(built_match.get("finish_date")) if built_match else None
            )
            if planned_finish and built_finish:
                email.delay_days = (built_finish - planned_finish).days

...
def validate_folder_path(path: str) -> str:
    ...
    if any(char in path for char in invalid_chars):
        raise HTTPException(400, "invalid path: contains forbidden characters")
...
def create_folder_record(db: Session, path: str, owner_user_id: str) -> Folder:
    ...
    existing_stmt = select(Folder).where(
        Folder.owner_user_id == owner_user_id, Folder.path == path
    )
    ...
    count_stmt = (
        ...
        .where(Document.owner_user_id == owner_user_id, Document.path == path)
    )
    ...
    folder = Folder(
        path=path, name=name, parent_path=parent_path, owner_user_id=owner_user_id
    )
    db.add(folder)
    return folder


def rename_folder_and_docs(
    db: Session, owner_user_id: str, old_path: str, new_name: str
) -> int:
    ...
    # Parameterized query via SQLAlchemy select()
    existing_stmt = select(Folder).where(
        Folder.owner_user_id == owner_user_id, Folder.path == new_path
    )
    ...
    count_stmt = (
        select(func.count())
        .select_from(Document)
        .where(Document.owner_user_id == owner_user_id, Document.path == new_path)
    )
    doc_count = db.execute(count_stmt).scalar() or 0
    ...
    folder_stmt = select(Folder).where(
        Folder.owner_user_id == owner_user_id, Folder.path == old_path
    )
    ...
    docs_stmt = select(Document).where(
        Document.owner_user_id == owner_user_id, Document.path == old_path
    )
    ...
    subdocs_stmt = select(Document).where(
        Document.owner_user_id == owner_user_id, Document.path.startswith(old_prefix)
    )
    ...
    subfolders_stmt = select(Folder).where(
        Folder.owner_user_id == owner_user_id, Folder.path.startswith(old_prefix)
    )
...
def delete_folder_and_docs(
    ...
    if total_docs > 0 and not recursive:
        raise HTTPException(
            400,
            f"folder contains {total_docs} document(s). Use recursive=true to delete with contents",
        )
    ...
    if recursive and total_docs > 0:
        # Parameterized query via SQLAlchemy select()
        docs_stmt = select(Document).where(
            Document.owner_user_id == owner_user_id, Document.path == path
        )
        docs = db.execute(docs_stmt).scalars().all()
    ...
    # Parameterized query via SQLAlchemy select()
    subfolders_stmt = select(Folder).where(
        Folder.owner_user_id == owner_user_id, Folder.path.startswith(path + "/")
    )
    subfolders = db.execute(subfolders_stmt).scalars().all()

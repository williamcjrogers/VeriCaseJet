...
def test_login_with_security():
    ...
    response = requests.post(
        f"{API_URL}/api/auth/login-secure",
        json={
...
def test_password_strength():
    ...
    passwords = ["weak", "Weak123", "Weak123!", "VeryStrongPassword123!"]

    for password in passwords:
        response = requests.post(
            f"{API_URL}/api/auth/check-password-strength", json=password
        )
        if response.status_code == 200:
            data = response.json()
...
def test_password_reset():
    ...
    # Request reset
    response = requests.post(
        f"{API_URL}/api/auth/request-reset", json={"email": "test@vericase.com"}
    )
...
def test_rate_limiting():
    ...
    for i in range(7):
        response = requests.post(
            f"{API_URL}/api/auth/login-secure",
            json={"email": "test@vericase.com", "password": "wrongpassword"},
        )

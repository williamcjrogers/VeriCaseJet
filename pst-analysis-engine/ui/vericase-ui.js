...
(function() {
    ...
    const ProgressTracker = {
        stages: [
            { id: 'profile', name: 'Create Profile', icon: 'fa-cog', url: 'wizard.html' },
            { id: 'upload', name: 'Upload PST', icon: 'fa-upload', url: 'pst-upload.html' },
            { id: 'refine', name: 'AI Refinement', icon: 'fa-magic', url: 'ai-refinement-wizard.html' },
            { id: 'evidence', name: 'Review Evidence', icon: 'fa-folder-open', url: 'evidence.html' },
            { id: 'analysis', name: 'Analysis', icon: 'fa-chart-line', url: 'correspondence-enterprise.html' }
        ],
        
        getProjectProgress(projectId) {
            const key = `vericase_progress_${projectId}`;
            const stored = localStorage.getItem(key);
            return stored ? JSON.parse(stored) : { completedStages: [], currentStage: 'profile' };
        },
    ...
    const Navigation = {
        ...
        init() {
            ...
            this.currentProject = urlParams.get('projectId') || localStorage.getItem('vericase_current_project');
        ...
        markActiveNavItem() {
            ...
            document.querySelectorAll('.nav-item').forEach(item => {
                ...
                if (href && currentPath.includes(href.replace('.html', '').toLowerCase())) {
        ...
        goToDashboard() { this.goTo('dashboard.html'); },
        goToEvidence() { this.goTo('evidence.html'); },
        goToCorrespondence() { this.goTo('correspondence-enterprise.html'); },
        goToUpload() { this.goTo('pst-upload.html'); },
        goToWizard() { this.goTo('wizard.html'); },
        goToRefinement() { this.goTo('ai-refinement-wizard.html'); }
    };
    ...
    const Loading = {
        ...
        showCardSkeleton(container, cards = 4) {
            ...
            let html = '<div class="loading-skeleton" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 16px;">';
    ...
    const Shortcuts = {
        ...
        init() {
            ...
            this.register('ctrl+/', () => document.querySelector('.search-input')?.focus());
        ...
        closeAll() {
            ...
            document.querySelectorAll('.slide-panel-backdrop.visible').forEach(b => b.classList.remove('visible'));

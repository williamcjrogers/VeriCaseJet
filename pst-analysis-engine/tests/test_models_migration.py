...
def test_model_imports():
    ...
    try:
        from api.app.models import (
            # Core user models
            User, UserSession, PasswordHistory, LoginAttempt,
            # Document models
            Document, DocStatus, Folder, ShareLink, UserInvitation,
            DocumentShare, FolderShare, Favorite, DocumentVersion,
            # Legal domain models
            Company, UserCompany, Case, CaseUser, Issue, Evidence,
            Claim, ChronologyItem, Rebuttal, ContractClause, SearchQuery, DelayEvent,
            # PST analysis models
            Project, PSTFile, EmailMessage, EmailAttachment,
            Stakeholder, Keyword, Programme, AppSetting,
            # Evidence repository models
            EvidenceSource, EvidenceCollection, EvidenceItem,
            EvidenceCorrespondenceLink, EvidenceRelation,
            EvidenceCollectionItem, EvidenceActivityLog,
            # Contentious matters models
            ContentiousMatter, HeadOfClaim, ItemClaimLink, ItemComment,
        )
...
def test_model_instantiation():
    ...
    try:
        from api.app.models import (
            User, Project, PSTFile, EmailMessage, EmailAttachment,
            Case, Company, Document, DocStatus
        )
        ...
        print(f"✓ EmailAttachment instantiated")
...
def test_property_aliases():
    ...
    try:
        ...
        assert pst.file_size == 1024, f"PSTFile.file_size alias failed: expected 1024, got {pst.file_size}"
        ...
        assert att.file_size == 2048, f"EmailAttachment.file_size alias failed: expected 2048, got {att.file_size}"
        print(f"✓ EmailAttachment.file_size alias works: {att.file_size} bytes")
        
        # Test None case
        att_none = EmailAttachment()
        assert att_none.file_size is None, "EmailAttachment.file_size should be None when file_size_bytes is None"
...
def test_database_schema():
    ...
    try:
        ...
        tables_to_check = [
            'users', 'projects', 'pst_files', 'email_messages', 
            'email_attachments', 'cases', 'companies', 'documents'
        ]
    ...
    try:
        ...
        try:
            ...
            if emails:
                ...
                print(f"  - has_attachments: {has_attachments} (type: {type(has_attachments).__name__})")
...
def test_relationships():
    ...
    try:
        from api.app.db import SessionLocal
        from api.app.models import EmailMessage, EmailAttachment, PSTFile, User
        ...
        try:
            ...
            if attachment:
                ...
                print(f"✓ attachment.email_message: {msg.subject[:30] if msg and msg.subject else None}...")
            else:
    ...
    try:
        ...
        try:
            ...
            if email:
                ...
                print(f"✓ Pydantic model created with ORM data")
                print(f"  - id: {summary.id}")
                print(f"  - subject: {summary.subject[:40] if summary.subject else None}...")
                print(f"  - has_attachments: {summary.has_attachments} (type: {type(summary.has_attachments).__name__})")
                ...
                print(f"✓ Pydantic model created with mock data")

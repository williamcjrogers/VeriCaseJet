...
from .models import EvidenceItem, EvidenceCorrespondenceLink, EmailMessage
        ...
        if extracted_text:
            ...
            for pattern, ref_type in REFERENCE_PATTERNS:
                ...
                for match in matches:
                    ref = (
                        f"{match[0]}-{match[1]}" if isinstance(match, tuple) else match
                    )
                    extracted_data["references"].append(
                        {"reference": ref.upper(), "type": ref_type, "confidence": 80}
                    )
                ...
                for match in matches:
                    ...
                    try:
                        ...
                        extracted_data["amounts"].append(
                            {"amount": amount, "currency": currency, "confidence": 70}
                        )
                    except ValueError:
                        pass
...
class EvidenceLinkingEngine:
    ...
    def find_correspondence_links(
        ...
        if evidence_item.source_email_id:
            email = (
                ...
                .filter(EmailMessage.id == evidence_item.source_email_id)
                .first()
            )
            if email:
                suggestions.append(
        ...
        filename_suggestions = self._find_by_filename_mention(
            evidence_item.filename, scope_case_id, scope_project_id
        )
        ...
        if evidence_item.extracted_references:
            ref_suggestions = self._find_by_references(
                evidence_item.extracted_references, scope_case_id, scope_project_id
            )
    ...
    def _find_by_filename_mention(
        self, filename: str, case_id: uuid.UUID | None, project_id: uuid.UUID | None
    ) -> list[LinkSuggestion]:
    ...
    def _find_by_date_proximity(
        ...
        query = self.db.query(EmailMessage).filter(
            and_(EmailMessage.date_sent >= date_from, EmailMessage.date_sent <= date_to)
        )
    ...
    def find_duplicate_evidence(
        ...
        query = self.db.query(EvidenceItem).filter(EvidenceItem.file_hash == file_hash)
    ...
    def find_related_evidence(
        self, evidence_item: EvidenceItem, max_results: int = 10
                ...
                items = (
                    ...
                    .filter(
                        and_(
                            ...
                            EvidenceItem.extracted_references.contains(
                                [{"reference": ref}]
                            ),
                        )
                    )
    ...
    def auto_link_evidence(
        ...
        if created_links:
            self.db.commit()
            logger.info(
                f"Auto-created {len(created_links)} links for evidence {evidence_item.id}"
            )

        return created_links

    def classify_and_enrich(self, evidence_item: EvidenceItem) -> dict[str, Any]:
        """
    ...
    for evidence_id in evidence_ids:
        try:
            ...
            duplicates = engine.find_duplicate_evidence(
                item.file_hash, exclude_id=item.id
            )
            if duplicates:

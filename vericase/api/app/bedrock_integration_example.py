...
List each point on a new line starting with a dash (-)."""
        ...
        response = await self.provider.invoke(
            model_id=self.default_model, prompt=prompt, max_tokens=500, temperature=0.5
        )
...
Explanation: [brief explanation]"""
        ...
        response = await self.provider.invoke(
            model_id=self.default_model, prompt=prompt, max_tokens=300, temperature=0.3
        )
...
Relationship: [how are they related?]"""
        ...
        return {"comparison": response, "model": "amazon.nova-lite-v1:0"}
...
Amounts: [monetary amounts or quantities]"""
        ...
        response = await self.provider.invoke(
            model_id=self.default_model, prompt=prompt, max_tokens=500, temperature=0.3
        )

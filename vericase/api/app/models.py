...
class WorkspaceAbout(Base):
    """Cached AI-generated 'About' context for a workspace (summary + Q&A scaffolding)."""

    __tablename__ = "workspace_about"
    ...
    # Authoritative user-provided framing (used as "ground truth" in prompts).
...
class WorkspacePurpose(Base):
    """Baseline purpose/instructions for a workspace with tracked deliverables."""

    __tablename__ = "workspace_purpose"
    ...
    instructions_evidence_id: Mapped[uuid.UUID | None] = mapped_column(
...
class ProjectIntelPack(Base):
    """Project-scoped intelligence pack (purpose baseline + tracking snapshot)."""

    __tablename__ = "project_intel_packs"
...
class ProjectIntelPackSnapshot(Base):
    """Immutable snapshot for project intel packs (audit trail)."""

    __tablename__ = "project_intel_pack_snapshots"

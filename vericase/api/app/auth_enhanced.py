...
def _create_session_token(
    user: User, remember_me: bool, client_ip: str, user_agent: str, db: Session
):
...
@router.post("/logout")
async def logout(
    user: User = Depends(current_user_enhanced), db: Session = Depends(get_db)
):
...
async def reset_password(data: PasswordResetConfirm, db: Session = Depends(get_db)):
    ...
    db.query(UserSession).filter(
        and_(UserSession.user_id == user.id, UserSession.revoked_at.is_(None))
...
async def verify_email(token: str = Path(...), db: Session = Depends(get_db)):

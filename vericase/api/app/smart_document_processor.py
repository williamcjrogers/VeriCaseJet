...
class SmartDocumentProcessor:
    ...
    async def process_document(
        ...
        try:
            ...
            results['document_type'] = await self._classify_document(results['extracted_text'])
    ...
    async def _classify_document(self, text: str) -> str:
        ...
        if any(word in text_lower for word in ['contract', 'agreement', 'parties agree']):
            return 'contract'
        ...
        elif any(word in text_lower for word in ['claim', 'damages', 'plaintiff', 'defendant']):
    ...
    async def _auto_populate_case(
        ...
        try:
            ...
            for org in results['entities'].get('organizations', [])[:5]:  # Limit to top 5
                self._add_stakeholder_if_not_exists(
                    db, case_id, org['text'], 'organization', org['score']
                )

            for person in results['entities'].get('persons', [])[:10]:  # Limit to top 10
                self._add_stakeholder_if_not_exists(
                    ...
                    **(document.meta or {}),
                    'smart_processing': {
                        ...
                        'entities_found': {
                            ...
                            'organizations': len(results['entities'].get('organizations', [])),
            ...
            db.commit()
            logger.info(f"Auto-populated case {case_id} with document {document_id} data")

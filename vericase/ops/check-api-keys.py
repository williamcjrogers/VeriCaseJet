...
def check_openai():
    ...
    try:
        ...
        return "✅ OpenAI key valid" if response.status_code == 200 else f"❌ OpenAI key invalid ({response.status_code})"
...
def check_anthropic():
    ...
    try:
        ...
        return "✅ Claude key valid" if response.status_code in [200, 400] else f"❌ Claude key invalid ({response.status_code})"
    except Exception as e:
...
def check_gemini():
    ...
    try:
        ...
        return "✅ Gemini key valid" if response.status_code == 200 else f"❌ Gemini key invalid ({response.status_code})"
    except Exception as e:
...
def check_xai():
    ...
    try:
        ...
        return "✅ xAI key valid" if response.status_code == 200 else f"❌ xAI key invalid ({response.status_code})"
    except Exception as e:
...
def check_perplexity():
    ...
    try:
        ...
        return "✅ Perplexity key valid" if response.status_code in [200, 400] else f"❌ Perplexity key invalid ({response.status_code})"
    except Exception as e:
...
def check_aws_bedrock():
    ...
    try:
        client = boto3.client('bedrock', region_name=os.getenv('AWS_REGION', 'eu-west-2'))
